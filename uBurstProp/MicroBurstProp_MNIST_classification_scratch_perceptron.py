...
num_images = 50 #len(X_train.T) # for short training
...
class Dense_uBurstProp(Layer):
    def __init__(self, input_units, output_units, next_layer_units=0, learning_rate=0.1):
        ...
        # A dense layer which performs: ... to fill in
    ...
    def backward(self, feedback, target=-1, BP_update=False):
        ...
        # output = np.dot(self.Y, self.B) + self.c
...
for epoch in range(num_epochs):
    ...
    # x_rand = X_train[:,indices_subsample]
    # y_rand = y_train[indices_subsample]

    # In each epoch use a new group of images (instead of randomizing)
    x_rand = X_train[:, epoch * num_images: (epoch + 1) * num_images]
    y_rand = y_train[epoch * num_images: (epoch + 1) * num_images]

    # Training on single image at a time
    for i_image in range(num_images):
        ER_rate.append(train(network, x_rand[:, i_image], y_rand[i_image], BP_update))
        if BP_update:
            BP_update = False

    # Calculate error rate
    train_log.append(np.mean(
        predict(network, X_train[:, 0:(num_epochs + 1) * num_images]) == y_train[0:(num_epochs + 1) * num_images]))
    val_log.append(
        np.mean(predict(network, X_val[:, 0:(num_epochs + 1) * num_images]) == y_val[0:(num_epochs + 1) * num_images]))
...
# Saving the figure
plt.savefig('Accuracy_Error_uBurstProp.png')
